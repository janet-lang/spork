...
static const uint8_t nibble_reverse_lut[16] = {0x0, 0x8, 0x4, 0xC, 0x2, 0xA,
...
static void crc8_make_variant(CRC8 *gen, uint8_t init, uint8_t polynomial,
                              bool byte_flip, uint8_t xor) {
  gen->init = init;
  gen->xor = xor;
  for (int i = 0; i < 256; i++) {
...
static uint16_t crc16_byte(uint16_t polynomial, uint8_t data) {
  ...
  for (int b = 16; b > 0; b--) {
    rem = (rem & 0x8000) ? (rem << 1) ^ polynomial : (rem << 1);
  }
  return rem;
}

static void crc16_make_variant(CRC16 *gen, uint16_t init, uint16_t polynomial,
                               bool byte_flip, uint16_t xor) {
  gen->init = init;
  gen->xor = xor;
...
static void crc32_make_variant(CRC32 *gen, uint32_t init, uint32_t polynomial,
                               bool byte_flip, uint32_t xor) {
  gen->init = init;
  gen->xor = xor;
  gen->flipped = byte_flip;
  for (int i = 0; i < 256; i++) {
...
static Janet crc8_call(void *variant, int32_t argc, Janet *argv) {
  ...
  return janet_wrap_integer(crc8_general(crc8, bytes.bytes + range.start,
...
static Janet crc16_call(void *variant, int32_t argc, Janet *argv) {
  ...
  return janet_wrap_integer(crc16_general(crc16, bytes.bytes + range.start,
                                          range.end - range.start, init));
}
...
static Janet crc32_call(void *variant, int32_t argc, Janet *argv) {
  ...
  return janet_wrap_number(crc32_general(crc32, bytes.bytes + range.start,
                                         range.end - range.start, init));
}

static const JanetAbstractType CRC8_AT = {.name = "crc/crc8-variant",
                                          .call = crc8_call};

static const JanetAbstractType CRC16_AT = {.name = "crc/crc16-variant",
                                           .call = crc16_call};

static const JanetAbstractType CRC32_AT = {.name = "crc/crc32-variant",
                                           .call = crc32_call};
...
JANET_FN(cfun_named_variant, "(crc/named-variant name)",
         "Get a named CRC variant.") {
  janet_fixarity(argc, 1);
  JanetKeyword kw = janet_getkeyword(argv, 0);
  const NamedVariant *nv = &named_variants[0];
  const NamedVariant *last_v =
      (NamedVariant *)((const char *)nv + sizeof(named_variants));
  while (nv < last_v) {
    if (0 == strcmp(nv->name, (const char *)kw))
      break;
    nv++;
  }
  if (nv > last_v)
    janet_panicf("unknown variant %v", argv[0]);
  void *ret;
  if (nv->size == 8) {
    ret = janet_abstract(&CRC8_AT, sizeof(CRC8));
    crc8_make_variant(ret, (uint8_t)nv->init, (uint8_t)nv->poly, nv->flipped,
                      (uint8_t)nv->xor);
  } else if (nv->size == 16) {
    ret = janet_abstract(&CRC16_AT, sizeof(CRC16));
    crc16_make_variant(ret, (uint16_t)nv->init, (uint16_t)nv->poly, nv->flipped,
                       (uint16_t)nv->xor);
  } else if (nv->size == 32) {
    ret = janet_abstract(&CRC32_AT, sizeof(CRC32));
    crc32_make_variant(ret, (uint32_t)nv->init, (uint32_t)nv->poly, nv->flipped,
                       (uint32_t)nv->xor);
  } else {
    janet_panic("nyi");
  }
...
JANET_FN(cfun_make_variant,
  ...
  if (size == 8) {
    ret = janet_abstract(&CRC8_AT, sizeof(CRC8));
    crc8_make_variant(ret, (uint8_t)init, (uint8_t)poly, byte_flip,
                      (uint8_t)xorout);
  } else if (size == 16) {
    ret = janet_abstract(&CRC16_AT, sizeof(CRC16));
    crc16_make_variant(ret, (uint16_t)init, (uint16_t)poly, byte_flip,
                       (uint16_t)xorout);
  } else {
    ret = janet_abstract(&CRC32_AT, sizeof(CRC32));
    crc32_make_variant(ret, (uint32_t)init, (uint32_t)poly, byte_flip,
                       (uint32_t)xorout);
  }
...
JANET_MODULE_ENTRY(JanetTable *env) {
  JanetRegExt cfuns[] = {JANET_REG("make-variant", cfun_make_variant),
