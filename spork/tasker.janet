...
(defdyn *queues* "Table of all created task queues")
(defdyn *task-dir* "Directory where task records are stored")
(defdyn *task-proc-map* "Map task ID to process objects for running tasks")
(defdyn *task-queued-map* "Map task ID to objects in tasker queues for running tasks")
...
(defn- run-task
  ...
  (put (dyn *task-proc-map*) task-id proc)
  (put (dyn *task-queued-map*) task-id payload)
  ...
  (try
    ...
    ([err]
     ...
     (os/proc-kill proc)))
  (put (dyn *task-proc-map*) task-id nil)
  (put (dyn *task-queued-map*) task-id nil)
...
(defn setup
  "Create queues and various settings to run tasks. Will set dynamic
  bindings in the current fiber."
  ...
  (def all-queues @{})
  (setdyn *queues* all-queues)
  (setdyn *task-dir* task-directory)
  (setdyn *task-proc-map* @{})
  (setdyn *task-queued-map* @{})
  (each q queues
...
(defn queue-task
  ...
  (def payload
    @{:task-id id 
      ...
      :meta-file meta-file
      :status :pending})
  (log "creating task directory %s" dir)
  (os/mkdir dir)
  (task-to-disk payload)
  (ev/give q payload)
  task-id)
...
(defn run-executors
  ...
  [&opt workers-per-queue pre-task post-task]
  (default workers-per-queue 1)
  (def nurse (ev-utils/nursery))
  (def qnames (distinct (map first (keys (dyn *queues*)))))
  (each qname qnames
    (def channels (seq [[[q p] c] :pairs (dyn *queues*) :when (= q qname)] c))
...
(defn close-queues
  ...
  []
  (each chan (dyn *queues*)
    (ev/chan-close chan)))

(defn task-status
  "Look up the status of a given task by id."
  [task-id]
  (parse (slurp (path/join (task-dir) task-id task-meta-name))))
...
(defn all-tasks
  ...
  [&opt detailed]
  (seq [dir :in (os/dir (task-dir)) :when (string/has-prefix? "task-" dir)]
    (if detailed (task-status dir) dir)))

(defn cancel-task
  "Cancel a queued or running task."
  [task-id]
  (def proc (get (dyn *task-proc-map* task-id)))
  (def payload (get (dyn *task-queued-map* task-id)))
...
(defn run-cleanup
  "Delete old jobs in the (dyn *task-dir*) directory."
  []
  (log "removing expired task records in " (task-dir))
  (each dir (os/dir (task-dir))
    (when (string/has-prefix? "task-" dir)
      (def meta-path (path/join (task-dir) dir task-meta-name))
      (try
        (do
          (def contents (-> meta-path slurp parse))
          (when (> (ts) (get contents :delete-after))
            (rm (path/join (task-dir) dir))))
        ([err]
         (log "failed to clean up task " dir " : " (describe err))))))
  (log "removed expired task records in " (task-dir)))
