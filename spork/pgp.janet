...
(defn hex->word
  ...
  [hex position] (get-in word-list [hex (% position 2)]))
...
(def- splitter
  (peg/compile
    ~{:boundaries (+ :s (set ",.?!_-/|\\"))
      :main (any (+ :boundaries '(some :a)))}))

(defn words->hexs
  "Returns an array of hexs from the string with pgp words."
  [words-string]
  (seq [w :in (peg/match splitter words-string)]
    (if-let [gw (word->hex w)]
